...
AUTOSTART_FILE="$HOME/.config/autostart/warp-tray.desktop"
BIN_FILE="$HOME/.local/bin/warp-tray"
...
need_elevate() {
    if [ "$(id -u)" -eq 0 ]; then
        ...
        echo "[warp-tray] Can't remove system package: no pkexec/sudo found." >&2
        return 1
    fi
}

log "Stopping tray if running..."
pkill -f "warp_tray.py" 2>/dev/null || true
