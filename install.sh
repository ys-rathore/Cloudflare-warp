...
DESKTOP_FILE="$AUTOSTART_DIR/cloudflare-warp.desktop"

log() { echo "[cloudflare-warp] $*"; }
die() { echo "[cloudflare-warp] ERROR: $*" >&2; exit 1; }
...
deploy_files() {
    ...
    cp -f "$SRC_DIR/cloudflare_warp.py" "$APP_DIR/"
    ...
    cat > "$BIN_DIR/cloudflare-warp" <<EOF
#!/usr/bin/env bash
exec python3 "$APP_DIR/cloudflare_warp.py"
EOF
    chmod +x "$BIN_DIR/cloudflare-warp"
...
start_now() {
    if pgrep -f "cloudflare_warp.py" >/dev/null 2>&1; then
        log "Tray already running, restarting it."
        pkill -f "cloudflare_warp.py" || true
        sleep 1
    fi
    nohup python3 "$APP_DIR/cloudflare_warp.py" >/tmp/cloudflare-warp.log 2>&1 &
    disown
    sleep 1
    if pgrep -f "cloudflare_warp.py" >/dev/null 2>&1; then
        log "Tray started. Look for the icon in your panel/tray."
    else
        log "Tray did not stay running. Check /tmp/cloudflare-warp.log for details."
    fi
}

main() {
    [ -f "$SRC_DIR/cloudflare_warp.py" ] || die "Run this from inside the cloned repo."
